<div class="p-0 table-responsive">
                                    <table class="table table-striped mb-0">
                                        ...
                                        <tbody>
                                            <?php 
                                            $strSQL = "SELECT * FROM rec_progress a INNER JOIN type_status_research b ON a.rp_progress_status = b.id_status_research WHERE a.rp_id_rs = '$id_rs' AND a.rp_progress_id = 'terminate' AND a.rp_delete_status = '0'";
                                            $resClosing = $db->fetch($strSQL, true, true);
                                            if(($resClosing) && ($resClosing['count'] > 0)){
                                                foreach ($resClosing['data'] as $row) {
                                                    ?>
                                                    <tr>
                                                        <td>
                                                            <?php 
                                                ...
                                                ?>
                                                <tr>
                                                    <td colspan="5" class="text-center">ไม่พบรายงานยุติโครงการ</td>
                                                </tr>
                                                <?php
                                            }
                                            ?>
